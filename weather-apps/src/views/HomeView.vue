<template>
  <div class="grid grid-cols-12 h-screen">
    <div class="col-span-9 grid grid-cols-12">
      <div class="col-span-2 flex justify-center py-8 px-4 bg-[#f6f7f8]">
        <div>
      ...
      <div class="col-span-10 p-8">
        <nav class="flex justify-between mb-10">
          <h1 class="text-xl uppercase font-bold">Weather App</h1>
          <button
            class="
              flex
        ...
        >
          <div class="bg-blue-200 col-span-6 p-8 rounded-3xl"
          v-for="(data, index) in computedHourly"
          :key="index">
            <div class=" flex mb-16">
              <div
                class="p-4 bg-white rounded-full flex justify-between items-center"
              >
                <img :src="`http://openweathermap.org/img/wn/${data.weather[0].icon}.png`" alt="">

              </div>
              <div class="ml-4">
                <span class="text-2xl"> {{currentD()}} </span>
                <p class="text-md"> {{  currentH(index + 1)  }} </p>
              </div>
            </div>   
            <div class="grid grid-cols-12 mb-16">
              <div class="col-span-6 flex items-start">
                <div class="mr-5">
                  <h3 class="text-5xl font-semibold mb-3"> {{Math.round(data.temp)}} ºC</h3>
                  <span class="text-md font-medium"> {{data.weather[0].description}} </span>
                </div>
                <div>
                  <small class="bg-white rounded-md p-1"> {{Math.round(data.uvi)}} UVI </small>
                </div>
              </div>
              <div class="col-span-6">
                  <img class="inline-block h-48 w-48" :src="require(`../assets/images/${data.weather[0].icon}.svg`)" alt="">
              </div>
            </div>
            <div class="grid grid-cols-3 gap-4">
              <div class="bg-[#1B273C] p-5 rounded-2xl text-white text-center hover:opacity-75 hover:scale-110 hover:-translate-y-1 hover:duration-300">
                <p class="text-xl text-white font-semibold mb-3">Pressure</p>
                <p class="text-2xl font-bold"> {{ data.pressure }} hPa </p>
              </div>
              <div class="bg-[#D0DF7A] p-5 rounded-2xl text-center hover:opacity-75 hover:scale-110 hover:-translate-y-1 hover:duration-300">
                <p class="text-xl font-semibold mb-3">Visibility</p>
                <p class="text-2xl font-bold"> {{ (data.visibility) / 1000 }}  km</p>
              </div>
              <div class="bg-white p-5 rounded-2xl text-center hover:opacity-75 hover:scale-110 hover:-translate-y-1 hover:duration-300">
                <p class="text-xl font-semibold mb-3">Humidity</p>
                <p class="text-2xl font-bold"> {{data.humidity}} %</p>
              </div>
            </div>
          </div>
        </div>
        <div class="grid grid-cols-12 gap-8 h-96 mt-12">
          <div class="col-span-8">
            <p class="text-xl">How’s the temperature today?</p>
            <div 
            class="grid grid-cols-8 gap-4 mx-4 my-5"
            >
              <div
              class="col-span-2 border-r ml-8"
              v-for="(value, index) in computedNext"
              :key="index"
              >
                <img class="h-24 w-24 my-5 hover:opacity-75 hover:scale-110 hover:-translate-y-1 hover:duration-300" :src="require(`../assets/images/${value.weather[0].icon}.svg`)" alt="">
                <p class="text-2xl"> {{Math.round(value.temp)}} ºC </p>
                <p class="text-sm mt-3"> {{  currentH(index + 3)  }} </p>
              </div>
            </div>
          </div>
          <div class="col-span-4 bg-red-700 h-80 py-10 px-7 rounded-3xl">
            <div>
              <h2 class="text-2xl font-semibold hover:text-white"> {{currentTomorrow()}} </h2>
    ...
    <div class="bg-[#F6F8FD] col-span-3 py-8 px-10" v-if="typeof current.weather != 'undefined'">
      <div class="flex justify-between border-b border-[#EFEFF1] pb-5 mb-5">
        <div>
          <div class="flex items-center">
            <h3 class="text-2xl font-semibold"> {{ weather.main }} </h3>
             <img class="bg-white rounded-full text-center ml-4" :src="`http://openweathermap.org/img/wn/${weather.icon}.png`" alt="">
          </div>
          
          <h4 class="text-base font-light"> {{currentDateTime()}} </h4>
          <span class="text-sm"> {{ city }} </span>
        </div>
        <div>
          <h2 class="text-[#6C5FF3] font-semibold text-5xl"> {{ Math.round(current.temp) }} ºC</h2>
         
        </div>
      </div>
      <div class=" mb-14">
        <h3 class="text-2xl font-bold uppercase"> {{weather.description}} </h3>
        <div class="flex justify-center">
          <img class="inline-block h-60 w-60" :src="require(`../assets/images/${weather.icon}.svg`)" alt="">
        </div>
        
      </div>
      <div class="bg-[#1D273B] flex items-center px-7 py-2 rounded-2xl">
        <div class="bg-[#F8D359] rounded-full p-4">
          <img :src="`http://openweathermap.org/img/wn/${weather.icon}.png`" alt="">
        </div>
        <div class="ml-7">
          <div class="flex mt-6 mb-1.5">
            <p class="text-white text-xl font-semibold mr-4"> {{Math.round(current.uvi)}} UVI</p>
            <p
              class="bg-[#2b792b] text-xs text-white pt-1.5 px-1.5 rounded-lg box-content"
              v-if=" current.uvi  "
            >
              Low
            </p>
            <p
              class="bg-[#dfe738] text-xs text-white py-1.5 px-1.5 rounded-lg box-content"
              v-else-if=" 2 > current.uvi < 6 "
            >
              Moderate
            </p>
            <p
              class="bg-[#d78f4b] text-xs text-white pt-1.5 px-1.5 rounded-lg box-content"
              v-else-if=" 5 > current.uvi < 8 "
            >
              High
            </p>
            <p
              class="bg-[#aa2424] text-xs text-white pt-1.5 px-1.5 rounded-lg box-content"
              v-else-if=" 7 > current.uvi < 11 "
            >
              Very High
            </p>
            <p
              class="bg-[#9235b7] text-xs text-white pt-1.5 px-1.5 rounded-lg box-content"
              v-else-if=" 10 > current.uvi "
            >
              Extreme
            </p>
          </div>
      ...
      <div
        class="bg-white flex items-center justify-around mb-4 rounded-2xl py-7"
        v-for="(value, index) in computedDaily"
        :key="index"
      >
        <div class="bg-[#75B8DF] ml-7 rounded-full p-2">
          <img :src="`http://openweathermap.org/img/wn/${value.weather[0].icon}.png`" alt="">
        </div>
        <div>
          <p class="text-xl"> {{Math.round(value.temp.day)}} ºC </p>
          <p class="text-xs font-extralight"> {{  currentDay(index + 1)  }} </p>
        </div>
        <div>
          <p class="text-[#6C5FF3] font-semibold text-xl"> {{Math.round(value.temp.min)}} ºC / {{Math.round(value.temp.max)}} ºC</p>
        </div>
      </div>
      <div>
