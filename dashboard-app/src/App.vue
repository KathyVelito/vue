...
export default {
  ...
  async created() {
    ...
    Object.entries(Countries).forEach( d => {
      console.log(d[1])
      const date = moment(d[1].date, "YYYYMMDD").format("MM/DD");
      const {
        NewConfirmed,
        TotalConfirmed,
        NewDeaths,
      ...
      this.arrPositive.push({ date, total: NewConfirmed });
      this.arrHospitalized.push({ date, total: TotalConfirmed });
      this.arrInIcu.push({ date, total: NewDeaths });
      this.arrOnVentilators.push({ date, total: TotalDeaths });
      this.arrRecovered.push({ date, total: NewRecovered });
      this.arrDeaths.push({ date, total: TotalRecovered });
    });
  }
};
</script>
